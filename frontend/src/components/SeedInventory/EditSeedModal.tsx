...
export const EditSeedModal: React.FC<EditSeedModalProps> = ({ isOpen, onClose, onSuccess, seed }) => {
  ...
  useEffect(() => {
    if (isOpen && seed) {
      fetchPlants();
      // Pre-populate form with seed data
      setFormData({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showError(errorData.error || 'Failed to update seed');
      }
    } catch (error) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Edit Seed">
