...
- **2026-04-24 - AUDIT-009 verified closed after activeBed refresh fix**. Seed-saving writes now refresh `activeBed` / `visibleBeds` so close-and-reopen reads canonical state instead of stale frontend bed props. User re-test reported the flow now appears to pass.
- **2026-04-24 - AUDIT-010 verified closed**. Duplicate-plan naming flow now appears to provide a sensible immediate naming path from the user side; the earlier rename/cleanup concern no longer appears active in the short verification lane.
- **2026-04-24 - Short verification lane functionally complete**. User re-tests now indicate `AUDIT-003`, `AUDIT-004`, `AUDIT-005`, `AUDIT-006`, `AUDIT-007`, `AUDIT-008`, `AUDIT-009`, `AUDIT-010`, `AUDIT-011`, `AUDIT-012`, and `AUDIT-014` are resolved from the user side. Remaining open work is now broader feature/user-journey coverage, intentional deferrals, and technical debt rather than this small-fix verification lane.
...
- **2026-04-26 - AUDIT-016 verified closed across all three calendar surfaces**. User re-test confirmed the calendar consistency work now reads correctly enough in practice, including the originally-missed clicked-day modal: repeated same-day same-bed same-plant rows now collapse instead of presenting noisy per-cell duplicates, bringing DayDetailModal into line with CalendarGrid pills and ListView grouping behavior.
- **2026-04-26 - AUDIT-017 verified closed on the dashboard surface**. `Needs Attention Today` now groups the same per-cell planting reminders that previously inflated one logical task into multiple rows. Backend grouping plus frontend grouped-row rendering / snooze fan-out brought the dashboard into line with the already-fixed calendar surfaces; user re-test reported the grouped task now looks good.
- **2026-04-26 - AUDIT-018 improved but left under monitoring**. Dashboard transplant-due suppression was narrowed to use linked `IndoorSeedStart.status` instead of the stale completion proxy. User re-test reported the dashboard "looks better," but this item remains open for continued observation before full closure.
- **2026-04-26 - AUDIT-019 logged as a new Indoor Starts placement-state UX follow-up**. After choosing a placement, the card action still reads `Plan Placement` instead of clearly reflecting that a location has already been chosen.
- **2026-04-27 - AUDIT-020 verified closed**. Auto-created indoor starts now default `Current location` to `windowsill` and persist the chosen bed into `destination_bed_ids`, so newly created cards show both a correct indoor location and the selected `Planned bed`. Legacy bad rows remain as-is for now; optional SQL cleanup was explicitly deferred.
- **2026-04-28 - AUDIT-021 logged as a new weather/property ZIP propagation regression**. User reported that a newly created and validated property ZIP did not pass through to Weather & Alerts. Code inspection found likely stale `useProperty` cache behavior plus several weather-aware consumers that still read only `localStorage.weatherZipCode`. Full handoff: `weather-property-zip-propagation-regression-finding.md`.
- **2026-04-28 - AUDIT-021 verified closed after second-pass retest**. First fix unified weather ZIP resolution but user retest found a fresh-user / ZIP-only validation gap. Second pass captured the ZIP before validation could rewrite the visible address and cleared/restored weather ZIP state on registration. User retest reported the flow now appears to work across Dashboard and Weather & Alerts.
