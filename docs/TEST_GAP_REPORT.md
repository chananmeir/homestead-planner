...
tests/
    ...
    space-calculations.spec.ts # 10 serial tests — SFG/MIG/Row/Intensive parity (see 10.15 below)
    ...
    indoor-seed-starts.spec.ts # 14 serial tests — ISS CRUD, germination, lifecycle, UI (see 10.13 below)
```
...
### 10.7 Nutrition Module (`nutrition.spec.ts`) — IMPLEMENTED

**File**: `frontend/tests/nutrition.spec.ts` (16 serial tests, ~21s)
**Commit**: `4af2902`

```
Suite 1: Dashboard API Endpoint Validation (5 tests)
  NUT-01: GET dashboard returns totals and by_source structure          PASSING
  NUT-02: GET dashboard without year defaults to current year           PASSING
  NUT-03: GET garden nutrition returns totals and by_plant              PASSING
  NUT-04: GET livestock nutrition returns totals                        PASSING
  NUT-05: GET tree nutrition returns totals                             PASSING

Suite 2: Nutrition Estimate API (2 tests)
  NUT-06: POST estimate with plan items returns nutrition totals        PASSING
  NUT-07: POST estimate validates required fields                      PASSING

Suite 3: Nutritional Data CRUD (5 tests)
  NUT-08: POST nutritional data creates user-specific entry             PASSING
  NUT-09: GET nutritional data returns created entry                    PASSING
  NUT-10: POST same source_type+source_id updates (upsert)             PASSING
  NUT-11: DELETE nutritional data removes entry                         PASSING
  NUT-12: DELETE non-existent nutritional data returns 404              PASSING

Suite 4: UI Rendering & Interaction (4 tests)
  NUT-13: Nutrition page renders header with year selector              PASSING
  NUT-14: Year selector changes displayed year                          PASSING
  NUT-15: Dashboard shows summary cards or no-data message              PASSING
  NUT-16: Dashboard requires authentication (401 without session)       PASSING
```

**Strategy**: API-first for endpoint validation + UI verification in Nutrition tab. Fresh user starts with zero nutrition data, so dashboard returns zeroes. Tests seed custom nutritional data via API and test estimate endpoint separately. Adaptive UI assertions handle both "no data" (empty user) and "has data" (summary cards) states.

**data-testid selectors added (9)**: `nutrition-year-selector`, `nutrition-export-csv`, `nutrition-summary-cards`, `nutrition-calories-card`, `nutrition-protein-card`, `nutrition-carbs-card`, `nutrition-fat-card`, `nutrition-source-breakdown`, `nutrition-breakdown` (NutritionalDashboard.tsx).

**Run commands:**
```bash
npx playwright test tests/nutrition.spec.ts          # Just nutrition
npx playwright test tests/nutrition.spec.ts --headed  # Watch mode
```
...
### 10.13 Indoor Seed Starts (`indoor-seed-starts.spec.ts`) — IMPLEMENTED AND PASSING

**Status**: All 14 tests passing (~25s). Implemented 2026-02-28.

**data-testid selectors added (5)**: `btn-import-from-garden`, `btn-start-seeds`, `iss-card-{id}`, `iss-stat-active`, `iss-stat-transplanted` (IndoorSeedStarts.tsx).

```
Suite 1: CRUD via API (6 tests)
  ISS-01: Create seed start via API (201, indoorStart wrapper, auto plantingEvent)
  ISS-02: Create second seed start, GET returns both
  ISS-03: GET single seed start by ID
  ISS-04: Update seed start — germination tracking (seedsGerminated=13/15 → rate ~86.67%)
  ISS-05: Status lifecycle — germinating → growing → transplanted (via /transplant endpoint)
  ISS-06: Delete seed start via API, verify gone (404)

Suite 2: Seed Quantity Calculation & Validation (4 tests)
  ISS-07: Calculate seed quantity API (desired=10, rate=85% → seedsToStart=14)
  ISS-08: Create seed start with invalid plant returns 404
  ISS-09: GET with status filter returns filtered results
  ISS-10: Auth required — unauthenticated returns 401

Suite 3: UI Rendering & Interaction (4 tests)
  ISS-11: Indoor Starts page renders header and buttons (Start Seeds, From Garden Plan, stat cards)
  ISS-12: Seed start card visible with status badge
  ISS-13: Filter buttons are visible and functional (All/Seeded/Germinating/Growing/Transplanted)
  ISS-14: Start Seeds button opens Add modal with plant selector
```

**Note**: API-first for CRUD + germination rate + status lifecycle. UI tests verify header, stat cards, filter buttons, seed start cards with status badges, and Add modal. Dedicated `iss_test_user` for isolation.
...
### 10.15 Space Calculation Verification (`space-calculations.spec.ts`) — IMPLEMENTED AND PASSING

**Status**: All 10 tests passing (~6s). Implemented 2026-02-28.

**No data-testid selectors needed** (API-only tests against `/api/spacing-calculator`).

```
Suite 1: Square Foot Gardening Method (4 tests)
  SC-01: SFG tomato — 1 per square, cellsPerPlant = 1.0 (perSquare=1, 4×4=16 plants)
  SC-02: SFG lettuce — 4 per square, cellsPerPlant = 0.25 (perSquare=4, 4×4=64 plants)
  SC-03: SFG carrot — 16 per square, cellsPerPlant = 0.0625 (perSquare=16, 4×4=256 plants)
  SC-04: SFG watermelon — 0.5 per square, cellsPerPlant = 2.0 (perSquare=0.5, 4×8=16 plants)

Suite 2: MIGardener Method (3 tests)
  SC-05: MIGardener tomato — row/plant spacing in range, numRows×plantsPerRow = totalPlants
  SC-06: MIGardener pepper — spacing values, consistency, plantsPerSqFt present
  SC-07: MIGardener carrot — dense sowing (≤8" rows, ≤4" apart), >100 plants in 4×8 bed

Suite 3: Row, Intensive & Cross-Method (3 tests)
  SC-08: Row tomato — 36" rows, 24" apart, 1 row × 4 plants = 4 total in 4×8 bed
  SC-09: Intensive tomato — 18" on-center hexagonal, reasonable totalPlants for 4×4 bed
  SC-10: Cross-method lettuce (4×8) — SFG=128, MIG≥Row, all methods return positive counts
```

**Note**: API-first strategy testing `/api/spacing-calculator` endpoint from `utilities_bp.py` (uses `garden_methods.py` spacing tables). SFG tests include parity verification via `1/perSquare = cellsPerPlant`. MIGardener/Row/Intensive tests verify mathematical consistency (numRows × plantsPerRow = totalPlants). Cross-method test confirms method discrimination (same plant, different densities). Dedicated `sc_test_user` for isolation. Frontend-backend parity for the calculation SERVICE is separately covered by 114 backend + 55 frontend unit tests.

### 10.16 Cross-Feature Integration Tests

**Status: IMPLEMENTED** — `frontend/tests/cross-feature-integration.spec.ts` (3 tests, ~11s)

| # | Test | Modules Chained | Key Assertions |
|---|------|----------------|----------------|
| CF-01 | Plan → Export → Place → Harvest — full pipeline | GardenPlan, PlantingEvent, PlantedItem (batch), SeasonProgress, Harvest, HarvestStats | Plan created (201), export creates events, batch place at (0,0)+(1,0), season progress totalPlanned≥4 + totalAdded≥2 + byPlanItemId tracking, harvest logged (5.5 lbs excellent), stats[tomato-1].count≥1 + total≥5.5 |
| CF-02 | Multi-bed succession — plan, export, place, verify progress | GardenBed (3x), GardenPlan (multi-bed), CalendarExport, PlantingEvent, PlantedItem (batch), SeasonProgress | 3 beds created, lettuce 60 plants / 4 successions / 3 beds, export creates 12 events (4×3), 4 unique dates 14 days apart, shared successionGroupId, batch place 5 in bed 1, progress totalPlanned≥60 + totalAdded≥5 + byPlanItemId placedSeason=5 |
| CF-03 | Seed saving — place, toggle, collect, verify inventory | PlantedItem, SeedSaving toggle, BedDetail, CollectSeeds, SeedInventory | Place tomato (growing), toggle saveForSeed=true → status='saving-seed', bed detail confirms persistence + seedsCollected=false, collect seeds (qty=3, rate=90%) → plantedItem.status='harvested' + seedInventory.isHomegrown=true + brand='Homegrown' + sourcePlantedItemId linked, seed appears in /api/my-seeds |
...
| Nutrition Module | `tests/nutrition.spec.ts` | 16 | **PASSING** (~21s) |
| Indoor Seed Starts | `tests/indoor-seed-starts.spec.ts` | 14 | **PASSING** (~25s) |
| Space Calculations | `tests/space-calculations.spec.ts` | 10 | **PASSING** (~6s) |
| Cross-Feature Integration | `tests/cross-feature-integration.spec.ts` | 3 | **PASSING** (~11s) |
| **Total implemented** | | **209+** | |
...
| Space Calculations | 12 | 10 (space-calculations) | 0 | 22 |
...
| Nutrition | - | 16 (nutrition) | 0 | 16 |
| Property Designer | - | 13 (property-designer) | 0 | 13 |
| Indoor Seed Starts | - | 14 (indoor-seed-starts) | 0 | 14 |
| Admin | - | 13 (admin) | 0 | 13 |
| Photos | 3 | - | - | 3 |
| Plant DB Sync | 4 | - | - | 4 |
| Integration Journeys | - | 3 (cross-feature-integration) | 0 | 3 |
