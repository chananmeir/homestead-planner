"""
Database migration script to add seed density fields to planting_event table.

This migration adds support for MIGardener seed-density planting method,
which models seed count and self-thinning instead of individual plant placement.

Run this script from the backend directory:
    python add_seed_density_fields.py
"""
...
def run_migration():
    ...
    try:
        ...
        print("Starting migration: Adding seed density fields to planting_event table...")
        ...
        new_columns = [
            ('planting_method', 'VARCHAR(50)', 'individual_plants'),
            ('quantity', 'INTEGER', None),
            ('spacing', 'REAL', None),
            ('seed_count', 'INTEGER', None),
            ('seed_density', 'REAL', None),
            ('row_length_inches', 'REAL', None),
            ('expected_germination_rate', 'REAL', None),
            ('expected_survival_rate', 'REAL', None),
            ('expected_final_count', 'INTEGER', None),
            ('harvest_method', 'VARCHAR(50)', None),
        ]
        ...
        if not migrations_needed:
            print("[SUCCESS] All seed density fields already exist. No migration needed.")
            conn.close()
            return True
        ...
        if all_added:
            print(f"[SUCCESS] Successfully added {len(migrations_needed)} seed density fields to planting_event table")
            print("\nNew columns:")
            for col_name, col_type, default_value in migrations_needed:
...
if __name__ == '__main__':
    print("=" * 60)
    print("Seed Density Migration Script")
    print("=" * 60)
